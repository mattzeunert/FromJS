...
})(stringTraceNotTripleEqual(typeof window === "undefined"?"undefined":stringTraceTypeOf(window),stringTrace("undefined"))?window:this,function(window,noGlobal){ // Support: Firefox 18+
...
makeArray:function(arr,results){var ret=results || [];if(arr != null){if(isArraylike(Object(arr))){jQuery.merge(ret,stringTraceTripleEqual(typeof arr === "undefined"?"undefined":stringTraceTypeOf(arr),stringTrace("string"))?[arr]:arr);}else {push.call(ret,arr);}}return ret;},inArray:function(elem,arr,i){return arr == null?-1:indexOf.call(arr,elem,i);},merge:function(first,second){var len=+second.length,j=0,i=first.length;for(;j < len;j++) {first[i++] = second[j];}first.length = i;return first;},grep:function(elems,callback,invert){var callbackInverse,matches=[],i=0,length=elems.length,callbackExpect=!invert; // Go through the array, only saving the items
...
Expr.find[stringTrace("TAG")] = support.getElementsByTagName?function(tag,context){if(stringTraceNotTripleEqual(typeof context.getElementsByTagName === "undefined"?"undefined":stringTraceTypeOf(context.getElementsByTagName),stringTrace("undefined"))){return context.getElementsByTagName(tag); // DocumentFragment nodes don't have gEBTN
...
stringTraceTripleEqual(typeof selector === "undefined"?"undefined":stringTraceTypeOf(selector),stringTrace("string")) && rneedsContext.test(selector)?jQuery(selector):selector || [],false).length;}}); // Initialize a jQuery object
...
}else if(jQuery.isFunction(selector)){return stringTraceNotTripleEqual(typeof rootjQuery.ready === "undefined"?"undefined":stringTraceTypeOf(rootjQuery.ready),stringTrace("undefined"))?rootjQuery.ready(selector): // Execute immediately if ready is not present
...
options = stringTraceTripleEqual(typeof options === "undefined"?"undefined":stringTraceTypeOf(options),stringTrace("string"))?optionsCache[options] || createOptions(options):jQuery.extend({},options);var  // Last fire value (for non-forgettable lists)
...
promise:function(type,obj){var tmp,count=1,defer=jQuery.Deferred(),elements=this,i=this.length,resolve=function(){if(! --count){defer.resolveWith(elements,[elements]);}};if(stringTraceNotTripleEqual(typeof type === "undefined"?"undefined":stringTraceTypeOf(type),stringTrace("string"))){obj = type;type = undefined;}type = type || stringTrace("fx");while(i--) {tmp = data_priv.get(elements[i],stringTraceAdd(type,stringTrace("queueHooks")));if(tmp && tmp.empty){count++;tmp.empty.add(resolve);}}resolve();return defer.promise(obj);}});var pnum=/[+-]?(?:\d*\.|)\d+(?:[eE][+-]?\d+|)/.source;var cssExpand=[stringTrace("Top"),stringTrace("Right"),stringTrace("Bottom"),stringTrace("Left")];var isHidden=function(elem,el){ // isHidden might be called from jQuery#filter function;
...
return stringTraceNotTripleEqual(typeof jQuery === "undefined"?"undefined":stringTraceTypeOf(jQuery),strundefined) && stringTraceNotTripleEqual(jQuery.event.triggered,e.type)?jQuery.event.dispatch.apply(elem,arguments):undefined;};} // Handle multiple events separated by a space
...
event = event[jQuery.expando]?event:new jQuery.Event(type,stringTraceTripleEqual(typeof event === "undefined"?"undefined":stringTraceTypeOf(event),stringTrace("object")) && event); // Trigger bitmask: & 1 for native handlers; & 2 for jQuery (always true)
...
return animation.progress(animation.opts.progress).done(animation.opts.done,animation.opts.complete).fail(animation.opts.fail).always(animation.opts.always);}jQuery.Animation = jQuery.extend(Animation,{tweener:function(props,callback){if(jQuery.isFunction(props)){callback = props;props = [stringTrace("*")];}else {props = props.split(stringTrace(" "));}var prop,index=0,length=props.length;for(;index < length;index++) {prop = props[index];tweeners[prop] = tweeners[prop] || [];tweeners[prop].unshift(callback);}},prefilter:function(callback,prepend){if(prepend){animationPrefilters.unshift(callback);}else {animationPrefilters.push(callback);}}});jQuery.speed = function(speed,easing,fn){var opt=speed && stringTraceTripleEqual(typeof speed === "undefined"?"undefined":stringTraceTypeOf(speed),stringTrace("object"))?jQuery.extend({},speed):{complete:fn || !fn && easing || jQuery.isFunction(speed) && speed,duration:speed,easing:fn && easing || easing && !jQuery.isFunction(easing) && easing};opt.duration = jQuery.fx.off?0:stringTraceTripleEqual(typeof opt.duration === "undefined"?"undefined":stringTraceTypeOf(opt.duration),stringTrace("number"))?opt.duration:opt.duration in jQuery.fx.speeds?jQuery.fx.speeds[opt.duration]:jQuery.fx.speeds._default; // Normalize opt.queue - true/undefined/null -> "fx"
...
// (#11426)
stringTraceTripleEqual(typeof xhr.responseText === "undefined"?"undefined":stringTraceTypeOf(xhr.responseText),stringTrace("string"))?{text:xhr.responseText}:undefined,xhr.getAllResponseHeaders());}}};}; // Listen to events
