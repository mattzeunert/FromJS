...
tokenCache(selector,groups).slice(0);};function toSelector(tokens){var i=0,len=tokens.length,selector=stringTrace("");for(;i < len;i++) {selector += tokens[i].value;}return selector;}function addCombinator(matcher,combinator,base){var dir=combinator.dir,checkNonElements=base && stringTraceTripleEqual(dir,stringTrace("parentNode")),doneName=done++;return stringTraceUseValue(combinator.first)? // Check against closest ancestor/preceding element
...
dirrunsUnique=dirruns += stringTraceUseValue(contextBackup == null)?1:Math.random() || 0.1,len=elems.length;if(outermost){outermostContext = stringTraceNotTripleEqual(context,document) && context;} // Add elements passing elementMatchers directly to results
...
matchedCount += i;if(bySet && stringTraceNotTripleEqual(i,matchedCount)){j = 0;while(stringTraceUseValue(matcher = setMatchers[j++])) {matcher(unmatched,setMatched,context,xml);}if(seed){ // Reintegrate element matches to eliminate the need for sorting
...
stringTraceUseValue(stringTraceTripleEqual(name,stringTrace("width")))?1:0,val=0;for(;i < 4;i += 2) { // Both box models exclude margin, so add it if we want it
...
includeWidth = stringTraceUseValue(includeWidth)?1:0;for(;i < 4;i += 2 - includeWidth) {which = cssExpand[i];attrs[stringTraceAdd(stringTrace("margin"),which)] = attrs[stringTraceAdd(stringTrace("padding"),which)] = type;}if(includeWidth){attrs.opacity = attrs.width = type;}return attrs;}function createTween(value,prop,animation){var tween,collection=(tweeners[prop] || []).concat(tweeners[stringTrace("*")]),index=0,length=collection.length;for(;index < length;index++) {if(tween = collection[index].call(animation,prop,value)){ // We're done with this property
...
if(s.data){cacheURL = s.url += stringTraceAdd(stringTraceUseValue(rquery.test(cacheURL))?stringTrace("&"):stringTrace("?"),s.data); // #9682: remove data so that it's not used in an eventual retry
...
if(jsonProp){s[jsonProp] = s[jsonProp].replace(rjsonp,stringTraceAdd(stringTrace("$1"),callbackName));}else if(stringTraceNotTripleEqual(s.jsonp,false)){s.url += stringTraceAdd(stringTraceAdd(stringTraceAdd(stringTraceUseValue(rquery.test(s.url))?stringTrace("&"):stringTrace("?"),s.jsonp),stringTrace("=")),callbackName);} // Use data converter to retrieve json after script execution
