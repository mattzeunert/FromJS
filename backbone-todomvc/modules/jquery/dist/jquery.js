 ...
 */(function(global,factory){if(stringTraceTripleEqual(typeof module,stringTrace("object")) && stringTraceTripleEqual(typeof module.exports,stringTrace("object"))){ // For CommonJS and CommonJS-like environments where a proper `window`
...
module.exports = global.document?factory(global,true):function(w){if(!w.document){throw new Error(stringTrace("jQuery requires a window with a document"));}return factory(w);};}else {factory(global);} // Pass this if window is not defined yet
})(stringTraceNotTripleEqual(typeof window,stringTrace("undefined"))?window:this,function(window,noGlobal){ // Support: Firefox 18+
...
num < 0?this[stringTraceAdd(num,this.length)]:this[num]: // Return all the elements in a clean array
...
each:function(callback,args){return jQuery.each(this,callback,args);},map:function(callback){return this.pushStack(jQuery.map(this,function(elem,i){return callback.call(elem,i,elem);}));},slice:function(){return this.pushStack(slice.apply(this,arguments));},first:function(){return this.eq(0);},last:function(){return this.eq(-1);},eq:function(i){var len=this.length,j=stringTraceAdd(+i,i < 0?len:0);return this.pushStack(j >= 0 && j < len?[this[j]]:[]);},end:function(){return this.prevObject || this.constructor(null);}, // For internal use only.
// Behaves like an Array's method, not like a jQuery method.
push:push,sort:arr.sort,splice:arr.splice};jQuery.extend = jQuery.fn.extend = function(){var options,name,src,copy,copyIsArray,clone,target=arguments[0] || {},i=1,length=arguments.length,deep=false; // Handle a deep copy situation
if(stringTraceTripleEqual(typeof target,stringTrace("boolean"))){deep = target; // Skip the boolean and the target
target = arguments[i] || {};i++;} // Handle case when target is a string or something (possible in deep copy)
if(stringTraceNotTripleEqual(typeof target,stringTrace("object")) && !jQuery.isFunction(target)){target = {};} // Extend jQuery itself if only one argument is passed
...
return stringTraceTripleEqual(typeof obj,stringTrace("object")) || stringTraceTripleEqual(typeof obj,stringTrace("function"))?class2type[toString.call(obj)] || stringTrace("object"):typeof obj;}, // Evaluates a script in a global context
...
trim:function(text){return text == null?stringTrace(""):stringTraceAdd(text,stringTrace("")).replace(rtrim,stringTrace(""));}, // results is for internal usage only
makeArray:function(arr,results){var ret=results || [];if(arr != null){if(isArraylike(Object(arr))){jQuery.merge(ret,stringTraceTripleEqual(typeof arr,stringTrace("string"))?[arr]:arr);}else {push.call(ret,arr);}}return ret;},inArray:function(elem,arr,i){return arr == null?-1:indexOf.call(arr,elem,i);},merge:function(first,second){var len=+second.length,j=0,i=first.length;for(;j < len;j++) {first[i++] = second[j];}first.length = i;return first;},grep:function(elems,callback,invert){var callbackInverse,matches=[],i=0,length=elems.length,callbackExpect=!invert; // Go through the array, only saving the items
...
// arguments.
proxy:function(fn,context){var tmp,args,proxy;if(stringTraceTripleEqual(typeof context,stringTrace("string"))){tmp = fn[context];context = fn;fn = tmp;} // Quick check to determine if target is callable, in the spec
...
return stringTraceNotTripleEqual(high,high) || escapedWhitespace?escaped:high < 0? // BMP codepoint
...
i,groups,old,nid,newContext,newSelector;if(stringTraceNotTripleEqual(context?context.ownerDocument || context:preferredDoc,document)){setDocument(context);}context = context || document;results = results || [];nodeType = context.nodeType;if(stringTraceNotTripleEqual(typeof selector,stringTrace("string")) || !selector || stringTraceNotTripleEqual(nodeType,1) && stringTraceNotTripleEqual(nodeType,9) && stringTraceNotTripleEqual(nodeType,11)){return results;}if(!seed && documentIsHTML){ // Try to shortcut find operations when possible (e.g., not under DocumentFragment)
...
if(stringTraceTripleEqual(nodeType,1) && stringTraceNotTripleEqual(context.nodeName.toLowerCase(),stringTrace("object"))){groups = tokenize(selector);if(old = context.getAttribute(stringTrace("id"))){nid = old.replace(rescape,stringTrace("\\$&"));}else {context.setAttribute(stringTrace("id"),nid);}nid = stringTraceAdd(stringTraceAdd(stringTrace("[id='"),nid),stringTrace("'] "));i = groups.length;while(i--) {groups[i] = stringTraceAdd(nid,toSelector(groups[i]));}newContext = rsibling.test(selector) && testContext(context.parentNode) || context;newSelector = groups.join(stringTrace(","));}if(newSelector){try{push.apply(results,newContext.querySelectorAll(newSelector));return results;}catch(qsaError) {}finally {if(!old){context.removeAttribute(stringTrace("id"));}}}}} // All others
 ...
 */function addHandle(attrs,handler){var arr=attrs.split(stringTrace("|")),i=attrs.length;while(i--) {Expr.attrHandle[arr[i]] = handler;}} /**
...
if(cur){while(cur = cur.nextSibling) {if(stringTraceTripleEqual(cur,b)){return -1;}}}return a?1:-1;} /**
 ...
 */function testContext(context){return context && stringTraceNotTripleEqual(typeof context.getElementsByTagName,stringTrace("undefined")) && context;} // Expose support vars for convenience
...
var documentElement=elem && (elem.ownerDocument || elem).documentElement;return documentElement?stringTraceNotTripleEqual(documentElement.nodeName,stringTrace("HTML")):false;}; /**
...
if(support.getById){Expr.find[stringTrace("ID")] = function(id,context){if(stringTraceNotTripleEqual(typeof context.getElementById,stringTrace("undefined")) && documentIsHTML){var m=context.getElementById(id); // Check parentNode to catch when Blackberry 4.6 returns
// nodes that are no longer in the document #6963
return m && m.parentNode?[m]:[];}};Expr.filter[stringTrace("ID")] = function(id){var attrId=id.replace(runescape,funescape);return function(elem){return stringTraceTripleEqual(elem.getAttribute(stringTrace("id")),attrId);};};}else { // Support: IE6/7
// getElementById is not reliable as a find shortcut
delete Expr.find[stringTrace("ID")];Expr.filter[stringTrace("ID")] = function(id){var attrId=id.replace(runescape,funescape);return function(elem){var node=stringTraceNotTripleEqual(typeof elem.getAttributeNode,stringTrace("undefined")) && elem.getAttributeNode(stringTrace("id"));return node && stringTraceTripleEqual(node.value,attrId);};};} // Tag
Expr.find[stringTrace("TAG")] = support.getElementsByTagName?function(tag,context){if(stringTraceNotTripleEqual(typeof context.getElementsByTagName,stringTrace("undefined"))){return context.getElementsByTagName(tag); // DocumentFragment nodes don't have gEBTN
}else if(support.qsa){return context.querySelectorAll(tag);}}:function(tag,context){var elem,tmp=[],i=0, // By happy coincidence, a (broken) gEBTN appears on DocumentFragment nodes too
results=context.getElementsByTagName(tag); // Filter out possible comments
if(stringTraceTripleEqual(tag,stringTrace("*"))){while(elem = results[i++]) {if(stringTraceTripleEqual(elem.nodeType,1)){tmp.push(elem);}}return tmp;}return results;}; // Class
...
contains = hasCompare || rnative.test(docElem.contains)?function(a,b){var adown=stringTraceTripleEqual(a.nodeType,9)?a.documentElement:a,bup=b && b.parentNode;return stringTraceTripleEqual(a,bup) || !!(bup && stringTraceTripleEqual(bup.nodeType,1) && (adown.contains?adown.contains(bup):a.compareDocumentPosition && a.compareDocumentPosition(bup) & 16));}:function(a,b){if(b){while(b = b.parentNode) {if(stringTraceTripleEqual(b,a)){return true;}}}return false;}; /* Sorting
...
compare = stringTraceTripleEqual(a.ownerDocument || a,b.ownerDocument || b)?a.compareDocumentPosition(b): // Otherwise we know they are disconnected
...
if(!aup || !bup){return stringTraceTripleEqual(a,doc)?-1:stringTraceTripleEqual(b,doc)?1:aup?-1:bup?1:sortInput?indexOf(sortInput,a) - indexOf(sortInput,b):0; // If the nodes are siblings, we can do a quick check
...
while(stringTraceTripleEqual(ap[i],bp[i])) {i++;}return i? // Do a sibling check if the nodes have a common ancestor
siblingCheck(ap[i],bp[i]): // Otherwise nodes in our document sort first
stringTraceTripleEqual(ap[i],preferredDoc)?-1:stringTraceTripleEqual(bp[i],preferredDoc)?1:0;};return doc;};Sizzle.matches = function(expr,elements){return Sizzle(expr,null,null,elements);};Sizzle.matchesSelector = function(elem,expr){ // Set document vars if needed
...
val=fn && hasOwn.call(Expr.attrHandle,name.toLowerCase())?fn(elem,name,!documentIsHTML):undefined;return stringTraceNotTripleEqual(val,undefined)?val:support.attributes || !documentIsHTML?elem.getAttribute(name):(val = elem.getAttributeNode(name)) && val.specified?val.value:null;};Sizzle.error = function(msg){throw new Error(stringTraceAdd(stringTrace("Syntax error, unrecognized expression: "),msg));}; /**
 * Document sorting and removing duplicates
 * @param {ArrayLike} results
 */Sizzle.uniqueSort = function(results){var elem,duplicates=[],j=0,i=0; // Unless we *know* we can detect duplicates, assume their presence
hasDuplicate = !support.detectDuplicates;sortInput = !support.sortStable && results.slice(0);results.sort(sortOrder);if(hasDuplicate){while(elem = results[i++]) {if(stringTraceTripleEqual(elem,results[i])){j = duplicates.push(i);}}while(j--) {results.splice(duplicates[j],1);}} // Clear input after sorting to release objects
...
ret += getText(node);}}else if(stringTraceTripleEqual(nodeType,1) || stringTraceTripleEqual(nodeType,9) || stringTraceTripleEqual(nodeType,11)){ // Use textContent for elements
// innerText usage removed for consistency of new lines (jQuery #11153)
if(stringTraceTripleEqual(typeof elem.textContent,stringTrace("string"))){return elem.textContent;}else { // Traverse its children
for(elem = elem.firstChild;elem;elem = elem.nextSibling) {ret += getText(elem);}}}else if(stringTraceTripleEqual(nodeType,3) || stringTraceTripleEqual(nodeType,4)){return elem.nodeValue;} // Do not include comment or processing instruction nodes
...
match[4] = +(match[4]?stringTraceAdd(match[5],match[6] || 1):2 * (stringTraceTripleEqual(match[3],stringTrace("even")) || stringTraceTripleEqual(match[3],stringTrace("odd"))));match[5] = +(stringTraceAdd(match[7],match[8]) || stringTraceTripleEqual(match[3],stringTrace("odd"))); // other types prohibit arguments
...
return match.slice(0,3);}},filter:{"TAG":function(nodeNameSelector){var nodeName=nodeNameSelector.replace(runescape,funescape).toLowerCase();return stringTraceTripleEqual(nodeNameSelector,stringTrace("*"))?function(){return true;}:function(elem){return elem.nodeName && stringTraceTripleEqual(elem.nodeName.toLowerCase(),nodeName);};},"CLASS":function(className){var pattern=classCache[stringTraceAdd(className,stringTrace(" "))];return pattern || (pattern = new RegExp(stringTraceAdd(stringTraceAdd(stringTraceAdd(stringTraceAdd(stringTraceAdd(stringTraceAdd(stringTrace("(^|"),whitespace),stringTrace(")")),className),stringTrace("(")),whitespace),stringTrace("|$)")))) && classCache(className,function(elem){return pattern.test(stringTraceTripleEqual(typeof elem.className,stringTrace("string")) && elem.className || stringTraceNotTripleEqual(typeof elem.getAttribute,stringTrace("undefined")) && elem.getAttribute(stringTrace("class")) || stringTrace(""));});},"ATTR":function(name,operator,check){return function(elem){var result=Sizzle.attr(elem,name);if(result == null){return stringTraceTripleEqual(operator,stringTrace("!="));}if(!operator){return true;}result += stringTrace("");return stringTraceTripleEqual(operator,stringTrace("="))?stringTraceTripleEqual(result,check):stringTraceTripleEqual(operator,stringTrace("!="))?stringTraceNotTripleEqual(result,check):stringTraceTripleEqual(operator,stringTrace("^="))?check && stringTraceTripleEqual(result.indexOf(check),0):stringTraceTripleEqual(operator,stringTrace("*="))?check && result.indexOf(check) > -1:stringTraceTripleEqual(operator,stringTrace("$="))?check && stringTraceTripleEqual(result.slice(-check.length),check):stringTraceTripleEqual(operator,stringTrace("~="))?stringTraceAdd(stringTraceAdd(stringTrace(" "),result.replace(rwhitespace,stringTrace(" "))),stringTrace(" ")).indexOf(check) > -1:stringTraceTripleEqual(operator,stringTrace("|="))?stringTraceTripleEqual(result,check) || stringTraceTripleEqual(result.slice(0,stringTraceAdd(check.length,1)),stringTraceAdd(check,stringTrace("-"))):false;};},"CHILD":function(type,what,argument,first,last){var simple=stringTraceNotTripleEqual(type.slice(0,3),stringTrace("nth")),forward=stringTraceNotTripleEqual(type.slice(-4),stringTrace("last")),ofType=stringTraceTripleEqual(what,stringTrace("of-type"));return stringTraceTripleEqual(first,1) && stringTraceTripleEqual(last,0)? // Shortcut for :nth-*(n)
function(elem){return !!elem.parentNode;}:function(elem,context,xml){var cache,outerCache,node,diff,nodeIndex,start,dir=stringTraceNotTripleEqual(simple,forward)?stringTrace("nextSibling"):stringTrace("previousSibling"),parent=elem.parentNode,name=ofType && elem.nodeName.toLowerCase(),useCache=!xml && !ofType;if(parent){ // :(first|last|only)-(child|of-type)
if(simple){while(dir) {node = elem;while(node = node[dir]) {if(ofType?stringTraceTripleEqual(node.nodeName.toLowerCase(),name):stringTraceTripleEqual(node.nodeType,1)){return false;}} // Reverse direction for :only-* (if we haven't yet done so)
start = dir = stringTraceTripleEqual(type,stringTrace("only")) && !start && stringTrace("nextSibling");}return true;}start = [forward?parent.firstChild:parent.lastChild]; // non-xml :nth-child(...) stores cache data on `parent`
if(forward && useCache){ // Seek `elem` from a previously-cached index
outerCache = parent[expando] || (parent[expando] = {});cache = outerCache[type] || [];nodeIndex = stringTraceTripleEqual(cache[0],dirruns) && cache[1];diff = stringTraceTripleEqual(cache[0],dirruns) && cache[2];node = nodeIndex && parent.childNodes[nodeIndex];while(node = ++nodeIndex && node && node[dir] || ( // Fallback to seeking `elem` from the start
...
while(node = ++nodeIndex && node && node[dir] || (diff = nodeIndex = 0) || start.pop()) {if((ofType?stringTraceTripleEqual(node.nodeName.toLowerCase(),name):stringTraceTripleEqual(node.nodeType,1)) && ++diff){ // Cache the index of each encountered element
...
var input=[],results=[],matcher=compile(selector.replace(rtrim,stringTrace("$1")));return matcher[expando]?markFunction(function(seed,matches,context,xml){var elem,unmatched=matcher(seed,null,xml,[]),i=seed.length; // Match elements unmatched by `matcher`
...
"first":createPositionalPseudo(function(){return [0];}),"last":createPositionalPseudo(function(matchIndexes,length){return [length - 1];}),"eq":createPositionalPseudo(function(matchIndexes,length,argument){return [argument < 0?stringTraceAdd(argument,length):argument];}),"even":createPositionalPseudo(function(matchIndexes,length){var i=0;for(;i < length;i += 2) {matchIndexes.push(i);}return matchIndexes;}),"odd":createPositionalPseudo(function(matchIndexes,length){var i=1;for(;i < length;i += 2) {matchIndexes.push(i);}return matchIndexes;}),"lt":createPositionalPseudo(function(matchIndexes,length,argument){var i=argument < 0?stringTraceAdd(argument,length):argument;for(;--i >= 0;) {matchIndexes.push(i);}return matchIndexes;}),"gt":createPositionalPseudo(function(matchIndexes,length,argument){var i=argument < 0?stringTraceAdd(argument,length):argument;for(;++i < length;) {matchIndexes.push(i);}return matchIndexes;})}};Expr.pseudos[stringTrace("nth")] = Expr.pseudos[stringTrace("eq")]; // Add button/input type pseudos
for(i in {radio:true,checkbox:true,file:true,password:true,image:true}) {Expr.pseudos[i] = createInputPseudo(i);}for(i in {submit:true,reset:true}) {Expr.pseudos[i] = createButtonPseudo(i);} // Easy API for creating new setFilters
function setFilters(){}setFilters.prototype = Expr.filters = Expr.pseudos;Expr.setFilters = new setFilters();tokenize = Sizzle.tokenize = function(selector,parseOnly){var matched,match,tokens,type,soFar,groups,preFilters,cached=tokenCache[stringTraceAdd(selector,stringTrace(" "))];if(cached){return parseOnly?0:cached.slice(0);}soFar = selector;groups = [];preFilters = Expr.preFilter;while(soFar) { // Comma and first run
...
tokenCache(selector,groups).slice(0);};function toSelector(tokens){var i=0,len=tokens.length,selector=stringTrace("");for(;i < len;i++) {selector += tokens[i].value;}return selector;}function addCombinator(matcher,combinator,base){var dir=combinator.dir,checkNonElements=base && stringTraceTripleEqual(dir,stringTrace("parentNode")),doneName=done++;return combinator.first? // Check against closest ancestor/preceding element
function(elem,context,xml){while(elem = elem[dir]) {if(stringTraceTripleEqual(elem.nodeType,1) || checkNonElements){return matcher(elem,context,xml);}}}: // Check against all ancestor/preceding elements
...
elems=seed || multipleContexts(selector || stringTrace("*"),context.nodeType?[context]:context,[]), // Prefilter to get matcher input, preserving a map for seed-results synchronization
...
}else {matcherOut = condense(stringTraceTripleEqual(matcherOut,results)?matcherOut.splice(preexisting,matcherOut.length):matcherOut);if(postFinder){postFinder(null,results,matcherOut,xml);}else {push.apply(results,matcherOut);}}});}function matcherFromTokens(tokens){var checkContext,matcher,j,len=tokens.length,leadingRelative=Expr.relative[tokens[0].type],implicitRelative=leadingRelative || Expr.relative[stringTrace(" ")],i=leadingRelative?1:0, // The foundational matcher ensures that elements are reachable from top-level context(s)
matchContext=addCombinator(function(elem){return stringTraceTripleEqual(elem,checkContext);},implicitRelative,true),matchAnyContext=addCombinator(function(elem){return indexOf(checkContext,elem) > -1;},implicitRelative,true),matchers=[function(elem,context,xml){var ret=!leadingRelative && (xml || stringTraceNotTripleEqual(context,outermostContext)) || ((checkContext = context).nodeType?matchContext(elem,context,xml):matchAnyContext(elem,context,xml)); // Avoid hanging onto element (issue #299)
checkContext = null;return ret;}];for(;i < len;i++) {if(matcher = Expr.relative[tokens[i].type]){matchers = [addCombinator(elementMatcher(matchers),matcher)];}else {matcher = Expr.filter[tokens[i].type].apply(null,tokens[i].matches); // Return special upon seeing a positional matcher
if(matcher[expando]){ // Find the next relative operator (if any) for proper handling
j = ++i;for(;j < len;j++) {if(Expr.relative[tokens[j].type]){break;}}return setMatcher(i > 1 && elementMatcher(matchers),i > 1 && toSelector( // If the preceding token was a descendant combinator, insert an implicit any-element `*`
tokens.slice(0,i - 1).concat({value:stringTraceTripleEqual(tokens[i - 2].type,stringTrace(" "))?stringTrace("*"):stringTrace("")})).replace(rtrim,stringTrace("$1")),matcher,i < j && matcherFromTokens(tokens.slice(i,j)),j < len && matcherFromTokens(tokens = tokens.slice(j)),j < len && toSelector(tokens));}matchers.push(matcher);}}return elementMatcher(matchers);}function matcherFromGroupMatchers(elementMatchers,setMatchers){var bySet=setMatchers.length > 0,byElement=elementMatchers.length > 0,superMatcher=function(seed,context,xml,results,outermost){var elem,j,matcher,matchedCount=0,i=stringTrace("0"),unmatched=seed && [],setMatched=[],contextBackup=outermostContext, // We must always have either seed elements or outermost context
...
dirrunsUnique=dirruns += contextBackup == null?1:Math.random() || 0.1,len=elems.length;if(outermost){outermostContext = stringTraceNotTripleEqual(context,document) && context;} // Add elements passing elementMatchers directly to results
// Keep `i` a string if there are no elements so `matchedCount` will be "00" below
// Support: IE<9, Safari
// Tolerate NodeList properties (IE: "length"; Safari: <number>) matching elements by id
for(;stringTraceNotTripleEqual(i,len) && (elem = elems[i]) != null;i++) {if(byElement && elem){j = 0;while(matcher = elementMatchers[j++]) {if(matcher(elem,context,xml)){results.push(elem);break;}}if(outermost){dirruns = dirrunsUnique;}} // Track unmatched elements for set filters
if(bySet){ // They will have gone through all possible matchers
if(elem = !matcher && elem){matchedCount--;} // Lengthen the array for every element, matched or not
if(seed){unmatched.push(elem);}}} // Apply set filters to unmatched elements
matchedCount += i;if(bySet && stringTraceNotTripleEqual(i,matchedCount)){j = 0;while(matcher = setMatchers[j++]) {matcher(unmatched,setMatched,context,xml);}if(seed){ // Reintegrate element matches to eliminate the need for sorting
 ...
 */select = Sizzle.select = function(selector,context,results,seed){var i,tokens,token,type,find,compiled=stringTraceTripleEqual(typeof selector,stringTrace("function")) && selector,match=!seed && tokenize(selector = compiled.selector || selector);results = results || []; // Try to minimize operations if there is no seed and only one group
...
i = matchExpr[stringTrace("needsContext")].test(selector)?0:tokens.length;while(i--) {token = tokens[i]; // Abort if we hit a combinator
...
if(!assert(function(div){return div.getAttribute(stringTrace("disabled")) == null;})){addHandle(booleans,function(elem,name,isXML){var val;if(!isXML){return stringTraceTripleEqual(elem[name],true)?name.toLowerCase():(val = elem.getAttributeNode(name)) && val.specified?val.value:null;}});}return Sizzle;})(window);jQuery.find = Sizzle;jQuery.expr = Sizzle.selectors;jQuery.expr[stringTrace(":")] = jQuery.expr.pseudos;jQuery.unique = Sizzle.uniqueSort;jQuery.text = Sizzle.getText;jQuery.isXMLDoc = Sizzle.isXML;jQuery.contains = Sizzle.contains;var rneedsContext=jQuery.expr.match.needsContext;var rsingleTag=/^<(\w+)\s*\/?>(?:<\/\1>|)$/;var risSimple=/^.[^:#\[\.,]*$/; // Implement the identical functionality for filter and not
function winnow(elements,qualifier,not){if(jQuery.isFunction(qualifier)){return jQuery.grep(elements,function(elem,i){ /* jshint -W018 */return stringTraceNotTripleEqual(!!qualifier.call(elem,i,elem),not);});}if(qualifier.nodeType){return jQuery.grep(elements,function(elem){return stringTraceNotTripleEqual(stringTraceTripleEqual(elem,qualifier),not);});}if(stringTraceTripleEqual(typeof qualifier,stringTrace("string"))){if(risSimple.test(qualifier)){return jQuery.filter(qualifier,elements,not);}qualifier = jQuery.filter(qualifier,elements);}return jQuery.grep(elements,function(elem){return stringTraceNotTripleEqual(indexOf.call(qualifier,elem) >= 0,not);});}jQuery.filter = function(expr,elems,not){var elem=elems[0];if(not){expr = stringTraceAdd(stringTraceAdd(stringTrace(":not("),expr),stringTrace(")"));}return stringTraceTripleEqual(elems.length,1) && stringTraceTripleEqual(elem.nodeType,1)?jQuery.find.matchesSelector(elem,expr)?[elem]:[]:jQuery.find.matches(expr,jQuery.grep(elems,function(elem){return stringTraceTripleEqual(elem.nodeType,1);}));};jQuery.fn.extend({find:function(selector){var i,len=this.length,ret=[],self=this;if(stringTraceNotTripleEqual(typeof selector,stringTrace("string"))){return this.pushStack(jQuery(selector).filter(function(){for(i = 0;i < len;i++) {if(jQuery.contains(self[i],this)){return true;}}}));}for(i = 0;i < len;i++) {jQuery.find(selector,self[i],ret);} // Needed because $( selector, context ) becomes $( context ).find( selector )
...
stringTraceTripleEqual(typeof selector,stringTrace("string")) && rneedsContext.test(selector)?jQuery(selector):selector || [],false).length;}}); // Initialize a jQuery object
...
if(stringTraceTripleEqual(typeof selector,stringTrace("string"))){if(stringTraceTripleEqual(selector[0],stringTrace("<")) && stringTraceTripleEqual(selector[selector.length - 1],stringTrace(">")) && selector.length >= 3){ // Assume that strings that start and end with <> are HTML and skip the regex check
...
}else if(jQuery.isFunction(selector)){return stringTraceNotTripleEqual(typeof rootjQuery.ready,stringTrace("undefined"))?rootjQuery.ready(selector): // Execute immediately if ready is not present
...
options = stringTraceTripleEqual(typeof options,stringTrace("string"))?optionsCache[options] || createOptions(options):jQuery.extend({},options);var  // Last fire value (for non-forgettable lists)
...
deferred[tuple[1]](function(){var returned=fn && fn.apply(this,arguments);if(returned && jQuery.isFunction(returned.promise)){returned.promise().done(newDefer.resolve).fail(newDefer.reject).progress(newDefer.notify);}else {newDefer[stringTraceAdd(tuple[0],stringTrace("With"))](stringTraceTripleEqual(this,promise)?newDefer.promise():this,fn?[returned]:arguments);}});});fns = null;}).promise();}, // Get a promise for this deferred
...
deferred[tuple[0]] = function(){deferred[stringTraceAdd(tuple[0],stringTrace("With"))](stringTraceTripleEqual(this,deferred)?promise:this,arguments);return this;};deferred[stringTraceAdd(tuple[0],stringTrace("With"))] = list.fireWith;}); // Make the deferred a promise
...
remaining=stringTraceNotTripleEqual(length,1) || subordinate && jQuery.isFunction(subordinate.promise)?length:0, // the master Deferred. If resolveValues consist of only a single Deferred, just use that.
deferred=stringTraceTripleEqual(remaining,1)?subordinate:jQuery.Deferred(), // Update function for both resolve and progress values
updateFunc=function(i,contexts,values){return function(value){contexts[i] = this;values[i] = arguments.length > 1?slice.call(arguments):value;if(stringTraceTripleEqual(values,progressValues)){deferred.notifyWith(contexts,values);}else if(! --remaining){deferred.resolveWith(contexts,values);}};},progressValues,progressContexts,resolveContexts; // Add listeners to Deferred subordinates; treat others as resolved
...
if(stringTraceTripleEqual(wait,true)?--jQuery.readyWait:jQuery.isReady){return;} // Remember that the DOM is ready
...
if(stringTraceTripleEqual(typeof data,stringTrace("string"))){cache[data] = value; // Handle: [ owner, { properties } ] args
...
var cache=this.cache[this.key(owner)];return stringTraceTripleEqual(key,undefined)?cache:cache[key];},access:function(owner,key,value){var stored; // In cases where either:
//
...
//
if(stringTraceTripleEqual(key,undefined) || key && stringTraceTripleEqual(typeof key,stringTrace("string")) && stringTraceTripleEqual(value,undefined)){stored = this.get(owner,key);return stringTraceNotTripleEqual(stored,undefined)?stored:this.get(owner,jQuery.camelCase(key));} // [*]When the key is not a string, or both a key and value
...
return stringTraceNotTripleEqual(value,undefined)?value:key;},remove:function(owner,key){var i,name,camel,unlock=this.key(owner),cache=this.cache[unlock];if(stringTraceTripleEqual(key,undefined)){this.cache[unlock] = {};}else { // Support array or space separated string of keys
...
if(stringTraceTripleEqual(data,undefined) && stringTraceTripleEqual(elem.nodeType,1)){name = stringTraceAdd(stringTrace("data-"),key.replace(rmultiDash,stringTrace("-$1")).toLowerCase());data = elem.getAttribute(name);if(stringTraceTripleEqual(typeof data,stringTrace("string"))){try{data = stringTraceTripleEqual(data,stringTrace("true"))?true:stringTraceTripleEqual(data,stringTrace("false"))?false:stringTraceTripleEqual(data,stringTrace("null"))?null: // Only convert to a number if it doesn't change the string
stringTraceTripleEqual(stringTraceAdd(+data,stringTrace("")),data)?+data:rbrace.test(data)?jQuery.parseJSON(data):data;}catch(e) {} // Make sure we set the data so it isn't changed later
...
if(stringTraceTripleEqual(key,undefined)){if(this.length){data = data_user.get(elem);if(stringTraceTripleEqual(elem.nodeType,1) && !data_priv.get(elem,stringTrace("hasDataAttrs"))){i = attrs.length;while(i--) { // Support: IE11+
...
if(stringTraceTripleEqual(typeof key,stringTrace("object"))){return this.each(function(){data_user.set(this,key);});}return access(this,function(value){var data,camelKey=jQuery.camelCase(key); // The calling jQuery object (element matches) is not empty
...
_queueHooks:function(elem,type){var key=stringTraceAdd(type,stringTrace("queueHooks"));return data_priv.get(elem,key) || data_priv.access(elem,key,{empty:jQuery.Callbacks(stringTrace("once memory")).add(function(){data_priv.remove(elem,[stringTraceAdd(type,stringTrace("queue")),key]);})});}});jQuery.fn.extend({queue:function(type,data){var setter=2;if(stringTraceNotTripleEqual(typeof type,stringTrace("string"))){data = type;type = stringTrace("fx");setter--;}if(arguments.length < setter){return jQuery.queue(this[0],type);}return stringTraceTripleEqual(data,undefined)?this:this.each(function(){var queue=jQuery.queue(this,type,data); // Ensure a hooks for this queue
...
promise:function(type,obj){var tmp,count=1,defer=jQuery.Deferred(),elements=this,i=this.length,resolve=function(){if(! --count){defer.resolveWith(elements,[elements]);}};if(stringTraceNotTripleEqual(typeof type,stringTrace("string"))){obj = type;type = undefined;}type = type || stringTrace("fx");while(i--) {tmp = data_priv.get(elements[i],stringTraceAdd(type,stringTrace("queueHooks")));if(tmp && tmp.empty){count++;tmp.empty.add(resolve);}}resolve();return defer.promise(obj);}});var pnum=/[+-]?(?:\d*\.|)\d+(?:[eE][+-]?\d+|)/.source;var cssExpand=[stringTrace("Top"),stringTrace("Right"),stringTrace("Bottom"),stringTrace("Left")];var isHidden=function(elem,el){ // isHidden might be called from jQuery#filter function;
...
stringTraceSetInnerHTML(div,stringTrace("<textarea>x</textarea>"));support.noCloneChecked = !!div.cloneNode(true).lastChild.defaultValue;})();var strundefined=typeof undefined;support.focusinBubbles = stringTrace("onfocusin") in window;var rkeyEvent=/^key/,rmouseEvent=/^(?:mouse|pointer|contextmenu)|click/,rfocusMorph=/^(?:focusinfocus|focusoutblur)$/,rtypenamespace=/^([^.]*)(?:\.(.+)|)$/;function returnTrue(){return true;}function returnFalse(){return false;}function safeActiveElement(){try{return document.activeElement;}catch(err) {}} /*
...
return stringTraceNotTripleEqual(typeof jQuery,strundefined) && stringTraceNotTripleEqual(jQuery.event.triggered,e.type)?jQuery.event.dispatch.apply(elem,arguments):undefined;};} // Handle multiple events separated by a space
types = (types || stringTrace("")).match(rnotwhite) || [stringTrace("")];t = types.length;while(t--) {tmp = rtypenamespace.exec(types[t]) || [];type = origType = tmp[1];namespaces = (tmp[2] || stringTrace("")).split(stringTrace(".")).sort(); // There *must* be a type, no attaching namespace-only handlers
...
types = (types || stringTrace("")).match(rnotwhite) || [stringTrace("")];t = types.length;while(t--) {tmp = rtypenamespace.exec(types[t]) || [];type = origType = tmp[1];namespaces = (tmp[2] || stringTrace("")).split(stringTrace(".")).sort(); // Unbind all events (on this namespace, if provided) for the element
if(!type){for(type in events) {jQuery.event.remove(elem,stringTraceAdd(type,types[t]),handler,selector,true);}continue;}special = jQuery.event.special[type] || {};type = (selector?special.delegateType:special.bindType) || type;handlers = events[type] || [];tmp = tmp[2] && new RegExp(stringTraceAdd(stringTraceAdd(stringTrace("(^|\\.)"),namespaces.join(stringTrace("\\.(?:.*\\.|)"))),stringTrace("(\\.|$)"))); // Remove matching events
...
if(jQuery.isEmptyObject(events)){delete elemData.handle;data_priv.remove(elem,stringTrace("events"));}},trigger:function(event,data,elem,onlyHandlers){var i,cur,tmp,bubbleType,ontype,handle,special,eventPath=[elem || document],type=hasOwn.call(event,stringTrace("type"))?event.type:event,namespaces=hasOwn.call(event,stringTrace("namespace"))?event.namespace.split(stringTrace(".")):[];cur = tmp = elem = elem || document; // Don't do events on text and comment nodes
...
event = event[jQuery.expando]?event:new jQuery.Event(type,stringTraceTripleEqual(typeof event,stringTrace("object")) && event); // Trigger bitmask: & 1 for native handlers; & 2 for jQuery (always true)
event.isTrigger = onlyHandlers?2:3;event.namespace = namespaces.join(stringTrace("."));event.namespace_re = event.namespace?new RegExp(stringTraceAdd(stringTraceAdd(stringTrace("(^|\\.)"),namespaces.join(stringTrace("\\.(?:.*\\.|)"))),stringTrace("(\\.|$)"))):null; // Clean up the event in case it is being reused
...
if(event.which == null){event.which = original.charCode != null?original.charCode:original.keyCode;}return event;}},mouseHooks:{props:stringTrace("button buttons clientX clientY offsetX offsetY pageX pageY screenX screenY toElement").split(stringTrace(" ")),filter:function(event,original){var eventDoc,doc,body,button=original.button; // Calculate pageX/Y if missing and clientX/Y available
...
if(!event.which && stringTraceNotTripleEqual(button,undefined)){event.which = button & 1?1:button & 2?3:button & 4?2:0;}return event;}},fix:function(event){if(event[jQuery.expando]){return event;} // Create a writable copy of the event object and normalize some properties
var i,prop,copy,type=event.type,originalEvent=event,fixHook=this.fixHooks[type];if(!fixHook){this.fixHooks[type] = fixHook = rmouseEvent.test(type)?this.mouseHooks:rkeyEvent.test(type)?this.keyHooks:{};}copy = fixHook.props?this.props.concat(fixHook.props):this.props;event = new jQuery.Event(originalEvent);i = copy.length;while(i--) {prop = copy[i];event[prop] = originalEvent[prop];} // Support: Cordova 2.5 (WebKit) (#13255)
// All events should have a target; Cordova deviceready doesn't
if(!event.target){event.target = document;} // Support: Safari 6.0+, Chrome<28
// Target should not be a text node (#504, #13143)
if(stringTraceTripleEqual(event.target.nodeType,3)){event.target = event.target.parentNode;}return fixHook.filter?fixHook.filter(event,originalEvent):event;},special:{load:{ // Prevent triggered image.load events from bubbling to window.load
noBubble:true},focus:{ // Fire native event if possible so blur/focus sequence is correct
trigger:function(){if(stringTraceNotTripleEqual(this,safeActiveElement()) && this.focus){this.focus();return false;}},delegateType:"focusin"},blur:{trigger:function(){if(stringTraceTripleEqual(this,safeActiveElement()) && this.blur){this.blur();return false;}},delegateType:"focusout"},click:{ // For checkbox, fire native event so checked state will be right
...
this.isDefaultPrevented = src.defaultPrevented || stringTraceTripleEqual(src.defaultPrevented,undefined) &&  // Support: Android<4.0
...
if(stringTraceTripleEqual(typeof types,stringTrace("object"))){ // ( types-Object, selector, data )
if(stringTraceNotTripleEqual(typeof selector,stringTrace("string"))){ // ( types-Object, data )
...
fn = selector;data = selector = undefined;}else if(fn == null){if(stringTraceTripleEqual(typeof selector,stringTrace("string"))){ // ( types, selector, fn )
...
handleObj = types.handleObj;jQuery(types.delegateTarget).off(handleObj.namespace?stringTraceAdd(stringTraceAdd(handleObj.origType,stringTrace(".")),handleObj.namespace):handleObj.origType,handleObj.selector,handleObj.handler);return this;}if(stringTraceTripleEqual(typeof types,stringTrace("object"))){ // ( types-object [, selector] )
for(type in types) {this.off(type,selector,types[type]);}return this;}if(stringTraceTripleEqual(selector,false) || stringTraceTripleEqual(typeof selector,stringTrace("function"))){ // ( types [, fn] )
...
function manipulationTarget(elem,content){return jQuery.nodeName(elem,stringTrace("table")) && jQuery.nodeName(stringTraceNotTripleEqual(content.nodeType,11)?content:content.firstChild,stringTrace("tr"))?elem.getElementsByTagName(stringTrace("tbody"))[0] || elem.appendChild(elem.ownerDocument.createElement(stringTrace("tbody"))):elem;} // Replace/restore the type attribute of script elements for safe DOM manipulation
...
if(data_user.hasData(src)){udataOld = data_user.access(src);udataCur = jQuery.extend({},udataOld);data_user.set(dest,udataCur);}}function getAll(context,tag){var ret=context.getElementsByTagName?context.getElementsByTagName(tag || stringTrace("*")):context.querySelectorAll?context.querySelectorAll(tag || stringTrace("*")):[];return stringTraceTripleEqual(tag,undefined) || tag && jQuery.nodeName(context,tag)?jQuery.merge([context],ret):ret;} // Fix IE bugs, see support tests
...
fragment.textContent = stringTrace("");i = 0;while(elem = nodes[i++]) { // #4087 - If origin and destination elements are the same, and this is
...
style.width = width;style.minWidth = minWidth;style.maxWidth = maxWidth;}}return stringTraceNotTripleEqual(ret,undefined)? // Support: IE
...
rdisplayswap=/^(none|table(?!-c[ea]).+)/,rnumsplit=new RegExp(stringTraceAdd(stringTraceAdd(stringTrace("^("),pnum),stringTrace(")(.*)$")),stringTrace("i")),rrelNum=new RegExp(stringTraceAdd(stringTraceAdd(stringTrace("^([+-])=("),pnum),stringTrace(")")),stringTrace("i")),cssShow={position:"absolute",visibility:"hidden",display:"block"},cssNormalTransform={letterSpacing:"0",fontWeight:"400"},cssPrefixes=[stringTrace("Webkit"),stringTrace("O"),stringTrace("Moz"),stringTrace("ms")]; // Return a css property mapped to a potentially vendor prefixed property
...
var capName=stringTraceAdd(name[0].toUpperCase(),name.slice(1)),origName=name,i=cssPrefixes.length;while(i--) {name = stringTraceAdd(cssPrefixes[i],capName);if(name in style){return name;}}return origName;}function setPositiveNumber(elem,value,subtract){var matches=rnumsplit.exec(value);return matches? // Guard against undefined "subtract", e.g., when used as in cssHooks
stringTraceAdd(Math.max(0,matches[1] - (subtract || 0)),matches[2] || stringTrace("px")):value;}function augmentWidthOrHeight(elem,name,extra,isBorderBox,styles){var i=stringTraceTripleEqual(extra,isBorderBox?stringTrace("border"):stringTrace("content"))? // If we already have the right measurement, avoid augmentation
4: // Otherwise initialize for horizontal or vertical properties
stringTraceTripleEqual(name,stringTrace("width"))?1:0,val=0;for(;i < 4;i += 2) { // Both box models exclude margin, so add it if we want it
if(stringTraceTripleEqual(extra,stringTrace("margin"))){val += jQuery.css(elem,stringTraceAdd(extra,cssExpand[i]),true,styles);}if(isBorderBox){ // border-box includes padding, so remove it if we want content
...
val += jQuery.css(elem,stringTraceAdd(stringTrace("padding"),cssExpand[i]),true,styles); // At this point, extra isn't content nor padding, so add border
if(stringTraceNotTripleEqual(extra,stringTrace("padding"))){val += jQuery.css(elem,stringTraceAdd(stringTraceAdd(stringTrace("border"),cssExpand[i]),stringTrace("Width")),true,styles);}}}return val;}function getWidthOrHeight(elem,name,extra){ // Start with offset property, which is equivalent to the border-box value
var valueIsBorderBox=true,val=stringTraceTripleEqual(name,stringTrace("width"))?elem.offsetWidth:elem.offsetHeight,styles=getStyles(elem),isBorderBox=stringTraceTripleEqual(jQuery.css(elem,stringTrace("boxSizing"),false,styles),stringTrace("border-box")); // Some non-html elements return undefined for offsetWidth, so check for null/undefined
...
// for such an element
if(stringTraceTripleEqual(elem.style.display,stringTrace("")) && isHidden(elem)){values[index] = data_priv.access(elem,stringTrace("olddisplay"),defaultDisplay(elem.nodeName));}}else {hidden = isHidden(elem);if(stringTraceNotTripleEqual(display,stringTrace("none")) || !hidden){data_priv.set(elem,stringTrace("olddisplay"),hidden?display:jQuery.css(elem,stringTrace("display")));}}} // Set the display of most of the elements in a second loop
// to avoid the constant reflow
for(index = 0;index < length;index++) {elem = elements[index];if(!elem.style){continue;}if(!show || stringTraceTripleEqual(elem.style.display,stringTrace("none")) || stringTraceTripleEqual(elem.style.display,stringTrace(""))){elem.style.display = show?values[index] || stringTrace(""):stringTrace("none");}}return elements;}jQuery.extend({ // Add in style property hooks for overriding the default
// behavior of getting and setting a style property
cssHooks:{opacity:{get:function(elem,computed){if(computed){ // We should always get a number back from opacity
var ret=curCSS(elem,stringTrace("opacity"));return stringTraceTripleEqual(ret,stringTrace(""))?stringTrace("1"):ret;}}}}, // Don't automatically add "px" to these possibly-unitless properties
...
if(stringTraceNotTripleEqual(value,undefined)){type = typeof value; // Convert "+=" or "-=" to relative numbers (#7345)
...
if(stringTraceTripleEqual(type,stringTrace("number")) && !jQuery.cssNumber[origName]){value += stringTrace("px");} // Support: IE9-11+
...
if(stringTraceTripleEqual(extra,stringTrace("")) || extra){num = parseFloat(val);return stringTraceTripleEqual(extra,true) || jQuery.isNumeric(num)?num || 0:val;}return val;}});jQuery.each([stringTrace("height"),stringTrace("width")],function(i,name){jQuery.cssHooks[name] = {get:function(elem,computed,extra){if(computed){ // Certain elements can have dimension info if we invisibly show them
// but it must have a current display style that would benefit
return rdisplayswap.test(jQuery.css(elem,stringTrace("display"))) && stringTraceTripleEqual(elem.offsetWidth,0)?jQuery.swap(elem,cssShow,function(){return getWidthOrHeight(elem,name,extra);}):getWidthOrHeight(elem,name,extra);}},set:function(elem,value,extra){var styles=extra && getStyles(elem);return setPositiveNumber(elem,value,extra?augmentWidthOrHeight(elem,name,extra,stringTraceTripleEqual(jQuery.css(elem,stringTrace("boxSizing"),false,styles),stringTrace("border-box")),styles):0);}};}); // Support: Android 2.3
jQuery.cssHooks.marginRight = addGetHookIf(support.reliableMarginRight,function(elem,computed){if(computed){return jQuery.swap(elem,{"display":"inline-block"},curCSS,[elem,stringTrace("marginRight")]);}}); // These hooks are used by animate to expand properties
jQuery.each({margin:"",padding:"",border:"Width"},function(prefix,suffix){jQuery.cssHooks[stringTraceAdd(prefix,suffix)] = {expand:function(value){var i=0,expanded={}, // Assumes a single number if not a string
parts=stringTraceTripleEqual(typeof value,stringTrace("string"))?value.split(stringTrace(" ")):[value];for(;i < 4;i++) {expanded[stringTraceAdd(stringTraceAdd(prefix,cssExpand[i]),suffix)] = parts[i] || parts[i - 2] || parts[0];}return expanded;}};if(!rmargin.test(prefix)){jQuery.cssHooks[stringTraceAdd(prefix,suffix)].set = setPositiveNumber;}});jQuery.fn.extend({css:function(name,value){return access(this,function(elem,name,value){var styles,len,map={},i=0;if(jQuery.isArray(name)){styles = getStyles(elem);len = name.length;for(;i < len;i++) {map[name[i]] = jQuery.css(elem,name[i],false,styles);}return map;}return stringTraceNotTripleEqual(value,undefined)?jQuery.style(elem,name,value):jQuery.css(elem,name);},name,value,arguments.length > 1);},show:function(){return showHide(this,true);},hide:function(){return showHide(this);},toggle:function(state){if(stringTraceTripleEqual(typeof state,stringTrace("boolean"))){return state?this.show():this.hide();}return this.each(function(){if(isHidden(this)){jQuery(this).show();}else {jQuery(this).hide();}});}});function Tween(elem,options,prop,end,easing){return new Tween.prototype.init(elem,options,prop,end,easing);}jQuery.Tween = Tween;Tween.prototype = {constructor:Tween,init:function(elem,options,prop,end,easing,unit){this.elem = elem;this.prop = prop;this.easing = easing || stringTrace("swing");this.options = options;this.start = this.now = this.cur();this.end = end;this.unit = unit || (jQuery.cssNumber[prop]?stringTrace(""):stringTrace("px"));},cur:function(){var hooks=Tween.propHooks[this.prop];return hooks && hooks.get?hooks.get(this):Tween.propHooks._default.get(this);},run:function(percent){var eased,hooks=Tween.propHooks[this.prop];if(this.options.duration){this.pos = eased = jQuery.easing[this.easing](percent,this.options.duration * percent,0,1,this.options.duration);}else {this.pos = eased = percent;}this.now = stringTraceAdd((this.end - this.start) * eased,this.start);if(this.options.step){this.options.step.call(this.elem,this.now,this);}if(hooks && hooks.set){hooks.set(this);}else {Tween.propHooks._default.set(this);}return this;}};Tween.prototype.init.prototype = Tween.prototype;Tween.propHooks = {_default:{get:function(tween){var result;if(tween.elem[tween.prop] != null && (!tween.elem.style || tween.elem.style[tween.prop] == null)){return tween.elem[tween.prop];} // Passing an empty string as a 3rd parameter to .css will automatically
...
return !result || stringTraceTripleEqual(result,stringTrace("auto"))?0:result;},set:function(tween){ // Use step hook for back compat.
...
tween.end = parts[1]?stringTraceAdd(start,stringTraceAdd(parts[1],1) * parts[2]):+parts[2];}return tween;}]}; // Animations created synchronously will run synchronously
...
includeWidth = includeWidth?1:0;for(;i < 4;i += 2 - includeWidth) {which = cssExpand[i];attrs[stringTraceAdd(stringTrace("margin"),which)] = attrs[stringTraceAdd(stringTrace("padding"),which)] = type;}if(includeWidth){attrs.opacity = attrs.width = type;}return attrs;}function createTween(value,prop,animation){var tween,collection=(tweeners[prop] || []).concat(tweeners[stringTrace("*")]),index=0,length=collection.length;for(;index < length;index++) {if(tween = collection[index].call(animation,prop,value)){ // We're done with this property
...
if(toggle){dataShow.hidden = !hidden;}if(hidden){jQuery(elem).show();}else {anim.done(function(){jQuery(elem).hide();});}anim.done(function(){var prop;data_priv.remove(elem,stringTrace("fxshow"));for(prop in orig) {jQuery.style(elem,prop,orig[prop]);}});for(prop in orig) {tween = createTween(hidden?dataShow[prop]:0,prop,anim);if(!(prop in dataShow)){dataShow[prop] = tween.start;if(hidden){tween.end = tween.start;tween.start = stringTraceTripleEqual(prop,stringTrace("width")) || stringTraceTripleEqual(prop,stringTrace("height"))?1:0;}}} // If this is a noop like .hide().hide(), restore an overwritten display value
}else if(stringTraceTripleEqual(stringTraceTripleEqual(display,stringTrace("none"))?defaultDisplay(elem.nodeName):display,stringTrace("inline"))){style.display = display;}}function propFilter(props,specialEasing){var index,name,easing,value,hooks; // camelCase, specialEasing and expand cssHook pass
...
return animation.progress(animation.opts.progress).done(animation.opts.done,animation.opts.complete).fail(animation.opts.fail).always(animation.opts.always);}jQuery.Animation = jQuery.extend(Animation,{tweener:function(props,callback){if(jQuery.isFunction(props)){callback = props;props = [stringTrace("*")];}else {props = props.split(stringTrace(" "));}var prop,index=0,length=props.length;for(;index < length;index++) {prop = props[index];tweeners[prop] = tweeners[prop] || [];tweeners[prop].unshift(callback);}},prefilter:function(callback,prepend){if(prepend){animationPrefilters.unshift(callback);}else {animationPrefilters.push(callback);}}});jQuery.speed = function(speed,easing,fn){var opt=speed && stringTraceTripleEqual(typeof speed,stringTrace("object"))?jQuery.extend({},speed):{complete:fn || !fn && easing || jQuery.isFunction(speed) && speed,duration:speed,easing:fn && easing || easing && !jQuery.isFunction(easing) && easing};opt.duration = jQuery.fx.off?0:stringTraceTripleEqual(typeof opt.duration,stringTrace("number"))?opt.duration:opt.duration in jQuery.fx.speeds?jQuery.fx.speeds[opt.duration]:jQuery.fx.speeds._default; // Normalize opt.queue - true/undefined/null -> "fx"
...
if(empty || data_priv.get(this,stringTrace("finish"))){anim.stop(true);}};doAnimation.finish = doAnimation;return empty || stringTraceTripleEqual(optall.queue,false)?this.each(doAnimation):this.queue(optall.queue,doAnimation);},stop:function(type,clearQueue,gotoEnd){var stopQueue=function(hooks){var stop=hooks.stop;delete hooks.stop;stop(gotoEnd);};if(stringTraceNotTripleEqual(typeof type,stringTrace("string"))){gotoEnd = clearQueue;clearQueue = type;type = undefined;}if(clearQueue && stringTraceNotTripleEqual(type,false)){this.queue(type || stringTrace("fx"),[]);}return this.each(function(){var dequeue=true,index=type != null && stringTraceAdd(type,stringTrace("queueHooks")),timers=jQuery.timers,data=data_priv.get(this);if(index){if(data[index] && data[index].stop){stopQueue(data[index]);}}else {for(index in data) {if(data[index] && data[index].stop && rrun.test(index)){stopQueue(data[index]);}}}for(index = timers.length;index--;) {if(stringTraceTripleEqual(timers[index].elem,this) && (type == null || stringTraceTripleEqual(timers[index].queue,type))){timers[index].anim.stop(gotoEnd);dequeue = false;timers.splice(index,1);}} // Start the next in the queue if the last step wasn't forced.
// Timers currently will call their complete callbacks, which
// will dequeue but only if they were gotoEnd.
if(dequeue || !gotoEnd){jQuery.dequeue(this,type);}});},finish:function(type){if(stringTraceNotTripleEqual(type,false)){type = type || stringTrace("fx");}return this.each(function(){var index,data=data_priv.get(this),queue=data[stringTraceAdd(type,stringTrace("queue"))],hooks=data[stringTraceAdd(type,stringTrace("queueHooks"))],timers=jQuery.timers,length=queue?queue.length:0; // Enable finishing flag on private data
...
delete data.finish;});}});jQuery.each([stringTrace("toggle"),stringTrace("show"),stringTrace("hide")],function(i,name){var cssFn=jQuery.fn[name];jQuery.fn[name] = function(speed,easing,callback){return speed == null || stringTraceTripleEqual(typeof speed,stringTrace("boolean"))?cssFn.apply(this,arguments):this.animate(genFx(name,true),speed,easing,callback);};}); // Generate shortcuts for custom animations
jQuery.each({slideDown:genFx(stringTrace("show")),slideUp:genFx(stringTrace("hide")),slideToggle:genFx(stringTrace("toggle")),fadeIn:{opacity:"show"},fadeOut:{opacity:"hide"},fadeToggle:{opacity:"toggle"}},function(name,props){jQuery.fn[name] = function(speed,easing,callback){return this.animate(props,speed,easing,callback);};});jQuery.timers = [];jQuery.fx.tick = function(){var timer,i=0,timers=jQuery.timers;fxNow = jQuery.now();for(;i < timers.length;i++) {timer = timers[i]; // Checks the timer has not already been removed
...
if(stringTraceTripleEqual(typeof elem.getAttribute,strundefined)){return jQuery.prop(elem,name,value);} // All attributes are lowercase
// Grab necessary hook if one is defined
if(stringTraceNotTripleEqual(nType,1) || !jQuery.isXMLDoc(elem)){name = name.toLowerCase();hooks = jQuery.attrHooks[name] || (jQuery.expr.match.bool.test(name)?boolHook:nodeHook);}if(stringTraceNotTripleEqual(value,undefined)){if(stringTraceTripleEqual(value,null)){jQuery.removeAttr(elem,name);}else if(hooks && stringTrace("set") in hooks && stringTraceNotTripleEqual(ret = hooks.set(elem,value,name),undefined)){return ret;}else {elem.setAttribute(name,stringTraceAdd(value,stringTrace("")));return value;}}else if(hooks && stringTrace("get") in hooks && stringTraceNotTripleEqual(ret = hooks.get(elem,name),null)){return ret;}else {ret = jQuery.find.attr(elem,name); // Non-existent attributes return null, we normalize to undefined
return ret == null?undefined:ret;}},removeAttr:function(elem,value){var name,propName,i=0,attrNames=value && value.match(rnotwhite);if(attrNames && stringTraceTripleEqual(elem.nodeType,1)){while(name = attrNames[i++]) {propName = jQuery.propFix[name] || name; // Boolean attributes get special treatment (#10870)
...
name = jQuery.propFix[name] || name;hooks = jQuery.propHooks[name];}if(stringTraceNotTripleEqual(value,undefined)){return hooks && stringTrace("set") in hooks && stringTraceNotTripleEqual(ret = hooks.set(elem,value,name),undefined)?ret:elem[name] = value;}else {return hooks && stringTrace("get") in hooks && stringTraceNotTripleEqual(ret = hooks.get(elem,name),null)?ret:elem[name];}},propHooks:{tabIndex:{get:function(elem){return elem.hasAttribute(stringTrace("tabindex")) || rfocusable.test(elem.nodeName) || elem.href?elem.tabIndex:-1;}}}});if(!support.optSelected){jQuery.propHooks.selected = {get:function(elem){var parent=elem.parentNode;if(parent && parent.parentNode){parent.parentNode.selectedIndex;}return null;}};}jQuery.each([stringTrace("tabIndex"),stringTrace("readOnly"),stringTrace("maxLength"),stringTrace("cellSpacing"),stringTrace("cellPadding"),stringTrace("rowSpan"),stringTrace("colSpan"),stringTrace("useMap"),stringTrace("frameBorder"),stringTrace("contentEditable")],function(){jQuery.propFix[this.toLowerCase()] = this;});var rclass=/[\t\r\n\f]/g;jQuery.fn.extend({addClass:function(value){var classes,elem,cur,clazz,j,finalValue,proceed=stringTraceTripleEqual(typeof value,stringTrace("string")) && value,i=0,len=this.length;if(jQuery.isFunction(value)){return this.each(function(j){jQuery(this).addClass(value.call(this,j,this.className));});}if(proceed){ // The disjunction here is for better compressibility (see removeClass)
classes = (value || stringTrace("")).match(rnotwhite) || [];for(;i < len;i++) {elem = this[i];cur = stringTraceTripleEqual(elem.nodeType,1) && (elem.className?stringTraceAdd(stringTraceAdd(stringTrace(" "),elem.className),stringTrace(" ")).replace(rclass,stringTrace(" ")):stringTrace(" "));if(cur){j = 0;while(clazz = classes[j++]) {if(cur.indexOf(stringTraceAdd(stringTraceAdd(stringTrace(" "),clazz),stringTrace(" "))) < 0){cur += stringTraceAdd(clazz,stringTrace(" "));}} // only assign if different to avoid unneeded rendering.
finalValue = jQuery.trim(cur);if(stringTraceNotTripleEqual(elem.className,finalValue)){elem.className = finalValue;}}}}return this;},removeClass:function(value){var classes,elem,cur,clazz,j,finalValue,proceed=stringTraceTripleEqual(arguments.length,0) || stringTraceTripleEqual(typeof value,stringTrace("string")) && value,i=0,len=this.length;if(jQuery.isFunction(value)){return this.each(function(j){jQuery(this).removeClass(value.call(this,j,this.className));});}if(proceed){classes = (value || stringTrace("")).match(rnotwhite) || [];for(;i < len;i++) {elem = this[i]; // This expression is here for better compressibility (see addClass)
cur = stringTraceTripleEqual(elem.nodeType,1) && (elem.className?stringTraceAdd(stringTraceAdd(stringTrace(" "),elem.className),stringTrace(" ")).replace(rclass,stringTrace(" ")):stringTrace(""));if(cur){j = 0;while(clazz = classes[j++]) { // Remove *all* instances
while(cur.indexOf(stringTraceAdd(stringTraceAdd(stringTrace(" "),clazz),stringTrace(" "))) >= 0) {cur = cur.replace(stringTraceAdd(stringTraceAdd(stringTrace(" "),clazz),stringTrace(" ")),stringTrace(" "));}} // Only assign if different to avoid unneeded rendering.
finalValue = value?jQuery.trim(cur):stringTrace("");if(stringTraceNotTripleEqual(elem.className,finalValue)){elem.className = finalValue;}}}}return this;},toggleClass:function(value,stateVal){var type=typeof value;if(stringTraceTripleEqual(typeof stateVal,stringTrace("boolean")) && stringTraceTripleEqual(type,stringTrace("string"))){return stateVal?this.addClass(value):this.removeClass(value);}if(jQuery.isFunction(value)){return this.each(function(i){jQuery(this).toggleClass(value.call(this,i,this.className,stateVal),stateVal);});}return this.each(function(){if(stringTraceTripleEqual(type,stringTrace("string"))){ // Toggle individual class names
...
this.className = this.className || stringTraceTripleEqual(value,false)?stringTrace(""):data_priv.get(this,stringTrace("__className__")) || stringTrace("");}});},hasClass:function(selector){var className=stringTraceAdd(stringTraceAdd(stringTrace(" "),selector),stringTrace(" ")),i=0,l=this.length;for(;i < l;i++) {if(stringTraceTripleEqual(this[i].nodeType,1) && stringTraceAdd(stringTraceAdd(stringTrace(" "),this[i].className),stringTrace(" ")).replace(rclass,stringTrace(" ")).indexOf(className) >= 0){return true;}}return false;}});var rreturn=/\r/g;jQuery.fn.extend({val:function(value){var hooks,ret,isFunction,elem=this[0];if(!arguments.length){if(elem){hooks = jQuery.valHooks[elem.type] || jQuery.valHooks[elem.nodeName.toLowerCase()];if(hooks && stringTrace("get") in hooks && stringTraceNotTripleEqual(ret = hooks.get(elem,stringTrace("value")),undefined)){return ret;}ret = elem.value;return stringTraceTripleEqual(typeof ret,stringTrace("string"))? // Handle most common string cases
ret.replace(rreturn,stringTrace("")): // Handle cases where value is null/undef or number
ret == null?stringTrace(""):ret;}return;}isFunction = jQuery.isFunction(value);return this.each(function(i){var val;if(stringTraceNotTripleEqual(this.nodeType,1)){return;}if(isFunction){val = value.call(this,i,jQuery(this).val());}else {val = value;} // Treat null/undefined as ""; convert numbers to string
if(val == null){val = stringTrace("");}else if(stringTraceTripleEqual(typeof val,stringTrace("number"))){val += stringTrace("");}else if(jQuery.isArray(val)){val = jQuery.map(val,function(value){return value == null?stringTrace(""):stringTraceAdd(value,stringTrace(""));});}hooks = jQuery.valHooks[this.type] || jQuery.valHooks[this.nodeName.toLowerCase()]; // If set returns undefined, fall back to normal setting
if(!hooks || !(stringTrace("set") in hooks) || stringTraceTripleEqual(hooks.set(this,val,stringTrace("value")),undefined)){this.value = val;}});}});jQuery.extend({valHooks:{option:{get:function(elem){var val=jQuery.find.attr(elem,stringTrace("value"));return val != null?val: // Support: IE10-11+
// option.text throws exceptions (#14686, #14858)
jQuery.trim(jQuery.text(elem));}},select:{get:function(elem){var value,option,options=elem.options,index=elem.selectedIndex,one=stringTraceTripleEqual(elem.type,stringTrace("select-one")) || index < 0,values=one?null:[],max=one?stringTraceAdd(index,1):options.length,i=index < 0?max:one?index:0; // Loop through all the selected options
...
support.optDisabled?!option.disabled:stringTraceTripleEqual(option.getAttribute(stringTrace("disabled")),null)) && (!option.parentNode.disabled || !jQuery.nodeName(option.parentNode,stringTrace("optgroup")))){ // Get the specific value for the option
...
jQuery.each([stringTrace("radio"),stringTrace("checkbox")],function(){jQuery.valHooks[this] = {set:function(elem,value){if(jQuery.isArray(value)){return elem.checked = jQuery.inArray(jQuery(elem).val(),value) >= 0;}}};if(!support.checkOn){jQuery.valHooks[this].get = function(elem){return stringTraceTripleEqual(elem.getAttribute(stringTrace("value")),null)?stringTrace("on"):elem.value;};}}); // Return jQuery for attributes-only inclusion
...
return stringTraceTripleEqual(arguments.length,1)?this.off(selector,stringTrace("**")):this.off(types,selector || stringTrace("**"),fn);}});var nonce=jQuery.now();var rquery=/\?/; // Support: Android 2.3
// Workaround failure to string-cast null input
jQuery.parseJSON = function(data){return JSON.parse(stringTraceAdd(data,stringTrace("")));}; // Cross-browser xml parsing
jQuery.parseXML = function(data){var xml,tmp;if(!data || stringTraceNotTripleEqual(typeof data,stringTrace("string"))){return null;} // Support: IE9
...
return function(dataTypeExpression,func){if(stringTraceNotTripleEqual(typeof dataTypeExpression,stringTrace("string"))){func = dataTypeExpression;dataTypeExpression = stringTrace("*");}var dataType,i=0,dataTypes=dataTypeExpression.toLowerCase().match(rnotwhite) || [];if(jQuery.isFunction(func)){ // For each dataType in the dataTypeExpression
...
function inspectPrefiltersOrTransports(structure,options,originalOptions,jqXHR){var inspected={},seekingTransport=stringTraceTripleEqual(structure,transports);function inspect(dataType){var selected;inspected[dataType] = true;jQuery.each(structure[dataType] || [],function(_,prefilterOrFactory){var dataTypeOrTransport=prefilterOrFactory(options,originalOptions,jqXHR);if(stringTraceTripleEqual(typeof dataTypeOrTransport,stringTrace("string")) && !seekingTransport && !inspected[dataTypeOrTransport]){options.dataTypes.unshift(dataTypeOrTransport);inspect(dataTypeOrTransport);return false;}else if(seekingTransport){return !(selected = dataTypeOrTransport);}});return selected;}return inspect(options.dataTypes[0]) || !inspected[stringTrace("*")] && inspect(stringTrace("*"));} // A special extend for ajax options
// that takes "flat" options (not to be deep extended)
// Fixes #9887
function ajaxExtend(target,src){var key,deep,flatOptions=jQuery.ajaxSettings.flatOptions || {};for(key in src) {if(stringTraceNotTripleEqual(src[key],undefined)){(flatOptions[key]?target:deep || (deep = {}))[key] = src[key];}}if(deep){jQuery.extend(true,target,deep);}return target;} /* Handles responses to an ajax request:
 * - finds the right dataType (mediates between content-type and expected dataType)
 * - returns the corresponding response
 */function ajaxHandleResponses(s,jqXHR,responses){var ct,type,finalDataType,firstDataType,contents=s.contents,dataTypes=s.dataTypes; // Remove auto dataType and get content-type in the process
while(stringTraceTripleEqual(dataTypes[0],stringTrace("*"))) {dataTypes.shift();if(stringTraceTripleEqual(ct,undefined)){ct = s.mimeType || jqXHR.getResponseHeader(stringTrace("Content-Type"));}} // Check if we're dealing with a known content-type
...
if(conv && s[stringTrace("throws")]){response = conv(response);}else {try{response = conv(response);}catch(e) {return {state:"parsererror",error:conv?e:stringTraceAdd(stringTraceAdd(stringTraceAdd(stringTrace("No conversion from "),prev),stringTrace(" to ")),current)};}}}}}}return {state:"success",data:response};}jQuery.extend({ // Counter for holding the number of active queries
...
if(stringTraceTripleEqual(typeof url,stringTrace("object"))){options = url;url = undefined;} // Force options to be an object
...
getResponseHeader:function(key){var match;if(stringTraceTripleEqual(state,2)){if(!responseHeaders){responseHeaders = {};while(match = rheaders.exec(responseHeadersString)) {responseHeaders[match[1].toLowerCase()] = match[2];}}match = responseHeaders[key.toLowerCase()];}return match == null?null:match;}, // Raw string
getAllResponseHeaders:function(){return stringTraceTripleEqual(state,2)?responseHeadersString:null;}, // Caches the header
...
if(s.crossDomain == null){parts = rurl.exec(s.url.toLowerCase());s.crossDomain = !!(parts && (stringTraceNotTripleEqual(parts[1],ajaxLocParts[1]) || stringTraceNotTripleEqual(parts[2],ajaxLocParts[2]) || stringTraceNotTripleEqual(parts[3] || (stringTraceTripleEqual(parts[1],stringTrace("http:"))?stringTrace("80"):stringTrace("443")),ajaxLocParts[3] || (stringTraceTripleEqual(ajaxLocParts[1],stringTrace("http:"))?stringTrace("80"):stringTrace("443")))));} // Convert data if not already a string
if(s.data && s.processData && stringTraceNotTripleEqual(typeof s.data,stringTrace("string"))){s.data = jQuery.param(s.data,s.traditional);} // Apply prefilters
...
if(s.data){cacheURL = s.url += stringTraceAdd(rquery.test(cacheURL)?stringTrace("&"):stringTrace("?"),s.data); // #9682: remove data so that it's not used in an eventual retry
delete s.data;} // Add anti-cache in url if needed
if(stringTraceTripleEqual(s.cache,false)){s.url = rts.test(cacheURL)? // If there is already a '_' parameter, set its value
cacheURL.replace(rts,stringTraceAdd(stringTrace("$1_="),nonce++)): // Otherwise add one to the end
stringTraceAdd(stringTraceAdd(stringTraceAdd(cacheURL,rquery.test(cacheURL)?stringTrace("&"):stringTrace("?")),stringTrace("_=")),nonce++);}} // Set the If-Modified-Since and/or If-None-Match header, if in ifModified mode.
...
jqXHR.setRequestHeader(stringTrace("Accept"),s.dataTypes[0] && s.accepts[s.dataTypes[0]]?stringTraceAdd(s.accepts[s.dataTypes[0]],stringTraceNotTripleEqual(s.dataTypes[0],stringTrace("*"))?stringTraceAdd(stringTraceAdd(stringTrace(", "),allTypes),stringTrace("; q=0.01")):stringTrace("")):s.accepts[stringTrace("*")]); // Check for headers option
...
jqXHR.statusCode(statusCode);statusCode = undefined;if(fireGlobals){globalEventContext.trigger(isSuccess?stringTrace("ajaxSuccess"):stringTrace("ajaxError"),[jqXHR,s,isSuccess?success:error]);} // Complete
...
buildParams(stringTraceAdd(stringTraceAdd(stringTraceAdd(prefix,stringTrace("[")),stringTraceTripleEqual(typeof v,stringTrace("object"))?i:stringTrace("")),stringTrace("]")),v,traditional,add);}});}else if(!traditional && stringTraceTripleEqual(jQuery.type(obj),stringTrace("object"))){ // Serialize object item.
...
value = jQuery.isFunction(value)?value():value == null?stringTrace(""):value;s[s.length] = stringTraceAdd(stringTraceAdd(encodeURIComponent(key),stringTrace("=")),encodeURIComponent(value));}; // Set traditional to true for jQuery <= 1.3.2 behavior.
...
var elements=jQuery.prop(this,stringTrace("elements"));return elements?jQuery.makeArray(elements):this;}).filter(function(){var type=this.type; // Use .is( ":disabled" ) so that fieldset[disabled] works
return this.name && !jQuery(this).is(stringTrace(":disabled")) && rsubmittable.test(this.nodeName) && !rsubmitterTypes.test(type) && (this.checked || !rcheckableType.test(type));}).map(function(i,elem){var val=jQuery(this).val();return val == null?null:jQuery.isArray(val)?jQuery.map(val,function(val){return {name:elem.name,value:val.replace(rCRLF,stringTrace("\r\n"))};}):{name:elem.name,value:val.replace(rCRLF,stringTrace("\r\n"))};}).get();}});jQuery.ajaxSettings.xhr = function(){try{return new XMLHttpRequest();}catch(e) {}};var xhrId=0,xhrCallbacks={},xhrSuccessStatus={ // file protocol always yields status code 0, assume 200
0:200, // Support: IE9
...
// (#11426)
stringTraceTripleEqual(typeof xhr.responseText,stringTrace("string"))?{text:xhr.responseText}:undefined,xhr.getAllResponseHeaders());}}};}; // Listen to events
...
if(s.crossDomain){var script,callback;return {send:function(_,complete){script = jQuery(stringTrace("<script>")).prop({async:true,charset:s.scriptCharset,src:s.url}).on(stringTrace("load error"),callback = function(evt){script.remove();callback = null;if(evt){complete(stringTraceTripleEqual(evt.type,stringTrace("error"))?404:200,evt.type);}});document.head.appendChild(script[0]);},abort:function(){if(callback){callback();}}};}});var oldCallbacks=[],rjsonp=/(=)\?(?=&|$)|\?\?/; // Default jsonp settings
jQuery.ajaxSetup({jsonp:"callback",jsonpCallback:function(){var callback=oldCallbacks.pop() || stringTraceAdd(stringTraceAdd(jQuery.expando,stringTrace("_")),nonce++);this[callback] = true;return callback;}}); // Detect, normalize options and install callbacks for jsonp requests
jQuery.ajaxPrefilter(stringTrace("json jsonp"),function(s,originalSettings,jqXHR){var callbackName,overwritten,responseContainer,jsonProp=stringTraceNotTripleEqual(s.jsonp,false) && (rjsonp.test(s.url)?stringTrace("url"):stringTraceTripleEqual(typeof s.data,stringTrace("string")) && !(s.contentType || stringTrace("")).indexOf(stringTrace("application/x-www-form-urlencoded")) && rjsonp.test(s.data) && stringTrace("data")); // Handle iff the expected data type is "jsonp" or we have a parameter to set
...
if(jsonProp){s[jsonProp] = s[jsonProp].replace(rjsonp,stringTraceAdd(stringTrace("$1"),callbackName));}else if(stringTraceNotTripleEqual(s.jsonp,false)){s.url += stringTraceAdd(stringTraceAdd(stringTraceAdd(rquery.test(s.url)?stringTrace("&"):stringTrace("?"),s.jsonp),stringTrace("=")),callbackName);} // Use data converter to retrieve json after script execution
...
jQuery.parseHTML = function(data,context,keepScripts){if(!data || stringTraceNotTripleEqual(typeof data,stringTrace("string"))){return null;}if(stringTraceTripleEqual(typeof context,stringTrace("boolean"))){keepScripts = context;context = false;}context = context || document;var parsed=rsingleTag.exec(data),scripts=!keepScripts && []; // Single tag
 ...
 */jQuery.fn.load = function(url,params,callback){if(stringTraceNotTripleEqual(typeof url,stringTrace("string")) && _load){return _load.apply(this,arguments);}var selector,type,response,self=this,off=url.indexOf(stringTrace(" "));if(off >= 0){selector = jQuery.trim(url.slice(off));url = url.slice(0,off);} // If it's a function
if(jQuery.isFunction(params)){ // We assume that it's the callback
callback = params;params = undefined; // Otherwise, build a param string
}else if(params && stringTraceTripleEqual(typeof params,stringTrace("object"))){type = stringTrace("POST");} // If we have elements to modify, make the request
...
parentOffset.top += jQuery.css(offsetParent[0],stringTrace("borderTopWidth"),true);parentOffset.left += jQuery.css(offsetParent[0],stringTrace("borderLeftWidth"),true);} // Subtract parent offsets and element margins
...
return rnumnonpx.test(computed)?stringTraceAdd(jQuery(elem).position()[prop],stringTrace("px")):computed;}});}); // Create innerHeight, innerWidth, height, width, outerHeight and outerWidth methods
jQuery.each({Height:"height",Width:"width"},function(name,type){jQuery.each({padding:stringTraceAdd(stringTrace("inner"),name),content:type,"":stringTraceAdd(stringTrace("outer"),name)},function(defaultExtra,funcName){ // Margin is only for outerHeight, outerWidth
jQuery.fn[funcName] = function(margin,value){var chainable=arguments.length && (defaultExtra || stringTraceNotTripleEqual(typeof margin,stringTrace("boolean"))),extra=defaultExtra || (stringTraceTripleEqual(margin,true) || stringTraceTripleEqual(value,true)?stringTrace("margin"):stringTrace("border"));return access(this,function(elem,type,value){var doc;if(jQuery.isWindow(elem)){ // As of 5/8/2012 this will yield incorrect results for Mobile Safari, but there
...
return Math.max(elem.body[stringTraceAdd(stringTrace("scroll"),name)],doc[stringTraceAdd(stringTrace("scroll"),name)],elem.body[stringTraceAdd(stringTrace("offset"),name)],doc[stringTraceAdd(stringTrace("offset"),name)],doc[stringTraceAdd(stringTrace("client"),name)]);}return stringTraceTripleEqual(value,undefined)? // Get width or height on the element, requesting but not forcing parseFloat
...
if(stringTraceTripleEqual(typeof define,stringTrace("function")) && define.amd){define(stringTrace("jquery"),[],function(){return jQuery;});}var  // Map over jQuery in case of overwrite
...
if(stringTraceTripleEqual(typeof noGlobal,strundefined)){window.jQuery = window.$ = jQuery;}return jQuery;});
